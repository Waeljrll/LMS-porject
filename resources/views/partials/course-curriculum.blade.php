@php
    $role = $role ?? (auth()->user()->isAdmin() ? 'admin' : 'instructor');

    $previewRoute = route('student.courses.show', $course->id) . '?preview=true';
@endphp
...
@if (auth()->user()->isAdmin() && session('error'))
    ...
    <div class="col-md-4 mb-4">
        ...
        @if (auth()->user()->isAdmin())
            <div class="card shadow-sm border-0 mt-3">
                <div class="card-header bg-{{ $course->status == 'published' ? 'success' : 'warning' }} text-white py-3">
            ...
            <div class="card-body">
                @if ($course->sections->count() == 0)
                    <div class="text-center py-5">
                        ...
                        <p class="text-muted small">استخدم الفورم الجانبية لإضافة الفصل الأول لتتمكن من رفع الدروس بداخله.</p>
                    </div>
                @else
                        ...
                        <div class="card mb-3 border-left-primary shadow-none bg-light">
                            <div class="card-header bg-white d-flex justify-content-between align-items-center py-3">
                                ...
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="btn-group">
                                        <form action="{{ route($role . '.sections.reorder', [$section->id, 'up']) }}" method="POST" class="d-inline">
                                            @csrf
                                            <button class="btn btn-outline-secondary btn-sm" title="Move Up"><i class="bi bi-arrow-up"></i></button>
                                        </form>
                                        <form action="{{ route($role . '.sections.reorder', [$section->id, 'down']) }}" method="POST" class="d-inline me-2">
                                            @csrf
                                            <button class="btn btn-outline-secondary btn-sm" title="Move Down"><i class="bi bi-arrow-down"></i></button>
                                        </form>
                                        ...
                                        @if ($section->lessons->count() > 0)
                                            <button class="btn btn-outline-danger btn-sm" disabled title="لا يمكن حذف فصل يحتوي على دروس!">
                                                <i class="bi bi-trash"></i>
                                            </button>
                                        @else
                                            <form action="{{ route($role . '.sections.destroy', $section->id) }}" method="POST" class="d-inline"
                                ...
                                <ul class="list-group list-group-flush">
                                    @if ($section->lessons->count() == 0)
                                        <li class="list-group-item text-muted text-center py-2 bg-light-soft">
                                            <small><i class="bi bi-info-circle me-1"></i> لا توجد دروس في هذا الفصل بعد.</small>
                                        </li>
                                    @else
                                            ...
                                            <li class="list-group-item d-flex justify-content-between align-items-center px-4 py-2">
                                                <div class="d-flex align-items-center">
                                                    ...
                                                    <div>
                                                        ...
                                                        <span class="text-muted small ms-2">({{ $lesson->duration_minutes }} mins)</span>
                                                ...
                                                <div class="btn-group">
                                                    ...
                                                    <form action="{{ route($role . '.lessons.destroy', $lesson->id) }}" method="POST" class="d-inline"
                                                        onsubmit="return confirm('هل أنت متأكد من حذف هذا الدرس؟ لا يمكن التراجع عن هذا الإجراء.')">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-sm btn-outline-danger" title="حذف الدرس">
                                                            <i class="bi bi-trash"></i>
                                                        </button>
                                                    </form>
                                                </div>
