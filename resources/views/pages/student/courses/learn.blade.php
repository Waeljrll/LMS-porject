            ...
            <div class="col-lg-8 mb-4">
                <div class="card shadow-sm border-0 h-100">
                    ...
                    <div class="card-footer bg-white border-top-0 p-4 d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            @if ($previousLesson)
                                <a href="{{ route('student.courses.learn', [$course, $previousLesson]) }}"
                        ...
                        <div>
                            @if ($nextLesson)
                                <a href="{{ route('student.courses.learn', [$course, $nextLesson]) }}"
                                ...
                                <div class="accordion-item border-bottom">
                                    ...
                                    <div id="collapse-{{ $section->id }}"
                                        ...
                                        <div class="list-group list-group-flush">
                                            ...
                                            {{-- حلقة عرض الدروس الخاصة بالقسم --}}
                                            @foreach ($section->lessons as $l)
                                                <a href="{{ route('student.courses.learn', [$course, $l]) }}"
                                                    class="list-group-item list-group-item-action py-3 d-flex justify-content-between align-items-center {{ $l->id == $lesson->id ? 'bg-primary bg-opacity-10 border-start border-primary border-4' : '' }}">
                                                    <div class="d-flex align-items-center">
                                                        {{-- Completion Icon --}}
                                                        <i
                                                            class="bi {{ auth()->user()->completedLessons->contains($l->id) ? 'bi-check-circle-fill text-success' : 'bi-circle text-muted' }} me-2"></i>
                                            ...
                                            {{-- الإضافة الاحترافية: زر الانتقال للاختبار التابع للقسم إن وجد --}}
                                            @if ($section->quiz)
                                                <a href="{{ route('student.quizzes.take', $section->quiz) }}"
                                                    class="list-group-item list-group-item-action py-3 d-flex justify-content-between align-items-center bg-warning bg-opacity-10 border-start border-warning border-4 text-decoration-none">
                                                    <div class="d-flex align-items-center">
                                                        <i class="fas fa-question-circle text-warning me-2 fs-5"></i>
                                                        <span class="fw-bold text-dark">
                                                            اختبار: {{ $section->quiz->title }}
                                                        </span>
                                                    </div>
                                                    <span
                                                        class="badge bg-warning text-dark border border-warning fw-bold px-2 py-1.5 rounded">
                                                        ابدأ الآن
                                                    </span>
                                                </a>
                                            @endif
