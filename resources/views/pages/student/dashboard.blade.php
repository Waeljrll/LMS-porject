                                ...
                                @foreach ($enrolledCourses->where('status', 'active')->take(3) as $enrollment)
                                    <div class="col-md-4 mb-3">
                                        <div class="card h-100 border-0 shadow-sm">
                                            ...
                                            <div class="card-body">
                                                <h6 class="card-title">{{ Str::limit($enrollment->course->title, 40) }}</h6>
                                                <p class="text-muted small mb-2">{{ $enrollment->course->instructor->name }}
                                                </p>

                                                <div class="progress mb-2" style="height: 8px;">
                                                    <div class="progress-bar bg-success" role="progressbar"
                                                        style="width: {{ $enrollment->actual_progress }}%"
                                                        aria-valuenow="{{ $enrollment->actual_progress }}"
                                                        aria-valuemin="0" aria-valuemax="100">
                                                    </div>
                                                </div>
                                                <div class="d-flex justify-content-between">
                                                    <small class="text-muted">{{ $enrollment->actual_progress }}%
                                                        complete</small>
                                                    <small
                                                        class="text-muted">{{ $enrollment->completed_lessons_count }}/{{ $enrollment->total_lessons_count }}
                                                        lessons</small>
                                                </div>
                                ...
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        @foreach ($enrolledCourses->take(5) as $enrollment)
                                            <tr>
                                                <td>
                                                    <img src="{{ $enrollment->course->thumbnail_url }}" width="40"
                                                ...
                                                <td>
                                                    <div class="progress" style="height: 6px; width: 100px;">
                                                        <div class="progress-bar" role="progressbar"
                                                            style="width: {{ $enrollment->actual_progress }}%"></div>
                                                    </div>
                                                    <small class="text-muted">{{ $enrollment->actual_progress }}%</small>
                                                </td>
