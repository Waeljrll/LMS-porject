...
@php
    $user = auth()->user();
    $myCourses = $user->courses()->with(['category', 'enrollments'])->latest()->get();
    $totalCourses = $myCourses->count();
    $totalStudents = \App\Models\Enrollment::whereIn('course_id', $myCourses->pluck('id'))->count();
    $avgRating = $myCourses->flatMap->reviews->avg('rating') ?? 0;
...
<section class="section dashboard">
    <div class="row">
            ...
            <div class="card info-card sales-card">
                <div class="card-body">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div class="ps-3">
                            <h6>{{ $totalCourses }}</h6>
                            <span class="text-muted small">{{ $publishedCourses }} published, {{ $draftCourses }} draft</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card info-card customers-card">
                <div class="card-body">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div class="ps-3">
                            <h6>{{ number_format($avgRating, 1) }} <small>/ 5.0</small></h6>
        ...
        <div class="col-lg-3 col-md-6">
            <div class="card info-card" style="background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%); color: white;">
                <div class="card-body">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div class="ps-3">
                            <h6 style="color: white;">${{ number_format($myCourses->sum('price'), 2) }}</h6>
                    ...
                    @if($totalCourses > 0)
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    @foreach($myCourses->take(5) as $course)
                                        @php
                                            $courseStudents = $course->enrollments->count();
                                            $courseRating = $course->reviews->avg('rating') ?? 0;
                                            $courseReviews = $course->reviews->count();
                                        @endphp
                                        <tr>
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    <img src="{{ $course->thumbnail_url }}" width="50" class="rounded me-2" style="object-fit: cover;">
                                                    <div>
                                                        <strong>{{ Str::limit($course->title, 30) }}</strong>
                                                        <br><small class="text-muted">{{ $course->lessons->count() }} lessons</small>
                                                    </div>
                                                </div>
                                            </td>
                                            <td><span class="badge bg-info">{{ $course->category->name }}</span></td>
                                            <td>
                                            ...
                                            <td>
                                                <i class="bi bi-people me-1"></i>{{ $courseStudents }}
                                            </td>
                                            <td>
                                                @if($courseReviews > 0)
                                                    ...
                                                    <small class="text-muted">({{ $courseReviews }})</small>
                                                @else
                                            ...
                                            <td>
                                                <div class="btn-group">
                                                    <a href="{{ route('instructor.courses.edit', $course->id) }}" class="btn btn-sm btn-outline-primary" title="Edit">
                                                        <i class="bi bi-pencil"></i>
                                                    </a>
                                                    <a href="{{ route('instructor.courses.content', $course->id) }}" class="btn btn-sm btn-outline-success" title="Content">
                        ...
                        @if($totalCourses > 5)
                            <div class="text-center mt-3">
                                <a href="{{ route('instructor.courses.my') }}" class="btn btn-outline-primary">
        ...
        <div class="col-lg-6 mt-4">
            <div class="card">
                <div class="card-body">
                    ...
                    @php
                        $recentEnrollments = \App\Models\Enrollment::whereIn('course_id', $myCourses->pluck('id'))
                            ->with(['student', 'course'])
                            ->latest()
                            ->take(5)
                            ->get();
                    ...
                    <div class="activity">
                        @foreach($recentEnrollments as $enrollment)
                            <div class="activity-item d-flex">
                                <div class="activite-label">{{ $enrollment->enrolled_at?->diffForHumans() ?? 'Recently' }}</div>
                                <i class="bi bi-circle-fill activity-badge text-success align-self-start"></i>
                                <div class="activity-content">
                                    <strong>{{ $enrollment->student->name }}</strong> enrolled in
                                    <a href="#" class="fw-bold text-dark">{{ Str::limit($enrollment->course->title, 25) }}</a>
                                </div>
                            </div>
                        @endforeach
                    </div>
        ...
        <div class="col-lg-6 mt-4">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="row g-3">
                        <div class="col-6">
                            <a href="{{ route('instructor.courses.create') }}" class="btn btn-outline-primary w-100 py-3">
                                <i class="bi bi-plus-circle fs-4 d-block mb-2"></i>
                                New Course
                            </a>
                        </div>
                        <div class="col-6">
                            <a href="{{ route('instructor.courses.my') }}" class="btn btn-outline-success w-100 py-3">
                                <i class="bi bi-journal-text fs-4 d-block mb-2"></i>
                                My Courses
                            </a>
                        </div>
                        <div class="col-6">
                            <a href="{{ route('instructor.courses.index') }}" class="btn btn-outline-info w-100 py-3">
