                ...
                <div class="card shadow-sm border-0">
                    <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                        <h5 class="mb-0 fw-bold text-dark">
                            <i class="bi bi-pencil-square text-primary me-2"></i> تعديل الدرس: {{ $lesson->title }}
                        </h5>
                        @php $role = auth()->user()->isAdmin() ? 'admin' : 'instructor'; @endphp
                        <a href="{{ route($role . '.courses.content', $lesson->section->course_id) }}"
                            class="btn btn-secondary btn-sm">
                            <i class="bi bi-arrow-left"></i> إلغاء ورجوع للمخطط
                        </a>
                    </div>
                    <div class="card-body">
                        ...
                        <form action="{{ route($role . '.lessons.update', $lesson->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="nav nav-pills nav-justified mb-3 card bg-light p-2 flex-row border-0"
                                ...
                                <button
                                    class="nav-link btn-sm py-2 {{ old('lesson_type', $lesson->lesson_type) === 'video' ? 'active bg-primary text-white' : 'text-dark' }}"
                                ...
                                <button
                                    class="nav-link btn-sm py-2 {{ old('lesson_type', $lesson->lesson_type) === 'document' ? 'active bg-primary text-white' : 'text-dark' }}"
                            ...
                            <input type="hidden" name="lesson_type" id="lesson_type_input"
                                value="{{ old('lesson_type', $lesson->lesson_type) }}">

                            <div class="tab-content" id="lessonTypeContent">

                                <div class="tab-pane fade {{ old('lesson_type', $lesson->lesson_type) === 'video' ? 'show active' : '' }}"
                                    id="panel-video" role="tabpanel">
                                    <div class="mb-3">
                                        <label class="form-label fw-bold text-danger">
                                            <i class="bi bi-link-45deg"></i> رابط الفيديو (Video URL)
                                        </label>
                                        <input type="url" name="video_url" class="form-control"
                                            value="{{ old('video_url', $lesson->video_url) }}"
                                            placeholder="https://example.com/video-link">
                                        <small class="text-muted d-block mt-1">أدخل رابط استضافة الفيديو المباشر الخاص
                                            بالدرس.</small>
                                    </div>
                                </div>

                                <div class="tab-pane fade {{ old('lesson_type', $lesson->lesson_type) === 'document' ? 'show active' : '' }}"
                            ...
                            <div class="form-check form-switch card bg-light p-3 my-3 border-0">
                                <div class="ms-4">
                                    ...
                                    <input class="form-check-input" type="checkbox" name="is_preview" id="is_preview"
                                        value="1" {{ old('is_preview', $lesson->is_preview) ? 'checked' : '' }}>
                            ...
                            <div class="text-end mt-4">
                                <button type="submit" class="btn btn-primary btn-sm px-4">حفظ التعديلات والعودة</button>
                            </div>
                        </form>
